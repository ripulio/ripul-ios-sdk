...
// knob schema (`RipulStyleKind.knobs`) and slots (`RipulStyleKind.slots`): a live example
// on top (the host's preview closure, re-rendered on every edit via
// `RipulThemeEngine.mergedStyle` / `mergedResolved`), a scrolling knob list below
// (Inherit/Set semantics — the style stays sparse). Field capsules, disclosure panels, and
// every FUTURE style kind get this screen from one init — there is no per-kind UI.
//
...
//
// `RipulStyleKnobRows` is the row rendering on its own — stateless, schema-driven — so a
// host can embed the SAME Inherit/Set rows inline (e.g. per-element override sections in
// its theme hub) without reimplementing the idiom.

// MARK: - Knob + slot rows (the reusable row set)

/// Schema-driven style rows for use inside a `Form`/`Section`: one row per slot (a style
/// picker over the slot kind's library) then one per knob (Inherit/Set semantics — the
/// dict stays sparse). Stateless: the parent owns the dict and receives every change.
@available(iOS 16.0, *)
@MainActor
public struct RipulStyleKnobRows: View {
    ...
    /// pointer unset (the element cascade or the slot's registered default decides). The
    /// choice lands in the dict as a string knob keyed by the slot's name — the document's
    /// slot-reference shape.
...
// MARK: - The pushed named-style editor
...
public struct RipulStyleKindEditorView<Preview: View, Footer: View>: View {
    ...
    public var body: some View {
        VStack(spacing: 0) {
            ...
            .background(.bar)
            Divider()
            // Scrolling rows beneath — the shared row set, split so composite parts lead.
            Form {
                if !slots.isEmpty {
                    Section("Parts") {
                        RipulStyleKnobRows(knobs: [], slots: slots, working: working,
                                           onChange: apply)
                    }
                }
                Section {
                    RipulStyleKnobRows(knobs: knobs, working: working, onChange: apply)
                } footer: {
    ...
    private func apply(_ next: [String: RipulKnob]) {
        working = next
        onChange(next)
    }
}
